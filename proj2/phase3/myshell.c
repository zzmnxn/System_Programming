...
char *insert_special_spaces(const char *cmdline) {
    char *result = malloc(strlen(cmdline) * 2 + 1); // 넉넉한 공간
    int i = 0, j = 0;
    while (cmdline[i]) {
...
int main() 
{
    ...
    setpgid(shell_pgid, shell_pgid);              // 자신을 그룹 리더로 설정
   ...
   if (fgets(cmdline, MAXLINE, stdin) == NULL) {
    ...
    char *formatted = insert_special_spaces(cmdline);
...
/*****************추가 */
    ...
    int fd[2];
    pid_t last_pid = 0; // 마지막 자식 pid 저장용
    ...
    for (i = 0; i < num_cmds; i++) {
        ...
        if (i == num_cmds - 1) {
            if(!bg) {
                fg_pid=pid; //foreground job 이면 저장
     ...
     if (!bg) {
        ...
        Job *job = getjob(last_pid); // pid는 마지막 프로세스
        if (job && job->state != Stopped) {
            deletejob(last_pid); // 종료된 경우만 삭제
        }
...
int builtin_command(char **argv) 
{
   ...
   exit(0);  
    ...
    if (!strcmp(argv[0], "fg")) {
        ...
        // ? SIGCHLD block 중이라면 아래처럼 unblock 필요
        sigset_t prev;
        Sigemptyset(&prev);
        PID = 0;
        while (!PID) {
            if (foreground_check()) Sigsuspend(&prev);
            else break;
        }
        // ? 다시 shell을 foreground로 복원
        if (tcsetpgrp(STDIN_FILENO, getpgrp()) < 0)
            perror("tcsetpgrp restore");

        fg_pid = 0;
        // job이 종료된 경우만 삭제 (Stopped 상태는 남겨야 함)
    Job *check = getjob(job->pid);
    if (!check || check->state != Stopped) deletejob(job->pid);
    ...
    if (!strcmp(argv[0], "kill")) {
        if (argv[1] == NULL || argv[1][0] != '%') {
            fprintf(stderr, "Kill error: job ID must start with '%%' (e.g., %%1)\n");
            fflush(stderr);
            return 1;
        }
    
        int jid = atoi(&argv[1][1]);  // % 기호 제거하고 숫자만 파싱
        Job *job = getjob_by_id(jid);
        if (!job) {
...
void sigchld_handler(int sig) {
    ...
    while ((pid = waitpid(-1, &status, WNOHANG | WUNTRACED|WCONTINUED)) > 0) {
        ...
        if (WIFEXITED(status) || WIFSIGNALED(status)) {
            if (pid != fg_pid)      //fg는 eval()에서 직접 처리
                deletejob(pid);
            else
                PID = pid;  
