...
struct bitmap *bitmap_expand(struct bitmap *bitmap, int size)
{
  ...
  size_t new_cnt = old_cnt + size;

  struct bitmap *new_bitmap = malloc(sizeof(struct bitmap));
  if (new_bitmap == NULL)
    return NULL;

  new_bitmap->bit_cnt = new_cnt;
  new_bitmap->bits = malloc(sizeof(elem_type) * elem_cnt(new_cnt));
  
  memset(new_bitmap->bits, 0, sizeof(elem_type) * elem_cnt(new_cnt));
